...
- Search by title (case-insensitive, debounced 300ms)
...
| Search/filter state | Plain `useState` in a dedicated hook (`useProductFilters`), with the search text debounced via `useDebounce` | Screen-local UI state; no need for a global store. Kept in its own hook (backed by pure functions in `utils/product-filters.ts`) purely so it's unit-testable in isolation |
...
src/
  ...
  hooks/          useProducts/useProduct (React Query), useProductFilters
                   (search/category logic), useDebounce, useTheme,
                   useColorScheme
...
- **Search** is a simple case-insensitive substring match on title, computed client-side over the already-fetched list (the API has no search parameter). Input is debounced 300ms (`useDebounce`) before filtering runs, so fast typing doesn't re-filter on every keystroke.
...
- **Test coverage**: 23 tests across 7 suites — filtering logic, API error handling, the favourites store, component render/interaction (`ProductCard`, `CategoryFilter`, `EmptyState`), and a navigation test (`app/__tests__/navigation.test.tsx`, via `expo-router/testing-library`) covering the product list → product details flow.
